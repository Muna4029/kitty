...
class SearchTreeNode(Generic[T]):
    ...
    def iter_token_nodes(self) -> Iterator[TokenNode[Any]]:
        return iter(())


class OrNode(SearchTreeNode[T]):

    def __init__(self, lhs: SearchTreeNode[Any], rhs: SearchTreeNode[Any]) -> None:
        self.lhs = lhs
        self.rhs = rhs
...
class NotNode(SearchTreeNode[T]):
    ...
    def __init__(self, rhs: SearchTreeNode[Any]) -> None:
        self.rhs = rhs
