...
read_fvar_font_table(const uint8_t *table, size_t table_len, PyObject *name_lookup_table, PyObject *output) {
    ...
    if (offset_to_start_of_axis_array > table_len || offset_to_start_of_axis_array + 8 > table_len) goto ok;
    uint32_t size_of_coordinates = (uint32_t)num_of_axis_records * sizeof(int32_t);
